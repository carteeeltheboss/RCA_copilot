...
class BaseProviderAdapter:
    ...
    def validate_config(self, config: dict[str, Any], settings: Settings) -> ValidationResult:
        ...
        if provider_type not in self.supported_provider_types:
            return ValidationResult(
                False, error=f"{self.provider_kind} does not support {provider_type}"
            )
        ...
        timeout = int(
            config.get("timeout_seconds") or settings.rca_provider_request_timeout_seconds
        )
        ...
        requested_capabilities = set(
            normalized.get("capabilities") or self.default_capabilities(provider_type)
        )
        invalid_capabilities = requested_capabilities - CAPABILITIES
        if invalid_capabilities:
            return ValidationResult(
                False, error=f"unsupported capabilities: {', '.join(sorted(invalid_capabilities))}"
            )
    ...
    async def test_connection(
        ...
        if normalized.get("extra_config", {}).get("offline_placeholder"):
            return ProviderResult(
                status="success",
                latency_ms=0,
                provider_identity=f"{self.provider_kind}:offline-placeholder",
            )
        if not normalized.get("base_url"):
            return ProviderResult(
                status="failure", latency_ms=0, error="provider base URL is not configured"
            )
        ...
        try:
            async with httpx.AsyncClient(
                timeout=float(
                    ...
                    or settings.rca_provider_request_timeout_seconds
                ),
                ...
                async with client.stream(
                    "GET", self.health_url(normalized["base_url"]), headers=self.headers(api_key)
                ) as response:
    ...
    async def list_models(
        self, config: dict[str, Any], settings: Settings, api_key: str | None = None
    ) -> ProviderResult:
    ...
    async def generate(
        ...
        return ProviderResult(
            status="unavailable", error="generation is not implemented for this provider yet"
        )
    ...
    async def embed(self, config: dict[str, Any], texts: list[str]) -> ProviderResult:
        ...
        return ProviderResult(
            status="unavailable", error="embedding is not implemented for this provider yet"
        )

    async def rerank(
        self, config: dict[str, Any], query: str, documents: list[dict[str, Any]]
    ) -> ProviderResult:
        if "reranker" not in self.capabilities:
            return unsupported("reranker")
        return ProviderResult(
            status="unavailable", error="reranking is not implemented for this provider yet"
        )
    ...
    def parse_response(self, body: bytes, latency_ms: int) -> ProviderResult:
        return ProviderResult(
            status="success", latency_ms=latency_ms, provider_identity=self.provider_kind
        )

...
class RCACopilotExplainer(MovingCameraScene):
    ...
    def problem(self) -> None:
        ...
        for line in samples:
            ...
            logs.add(Text(line, font=FONT, font_size=22, color=color))
        logs.arrange(DOWN, aligned_edge=LEFT, buff=0.21).move_to(stream_box.get_center()).align_to(stream_box, LEFT).shift(RIGHT * 0.35)
    ...
    def events_to_graph(self) -> None:
        ...
        edges = VGroup(
            Line(nodes[0].get_center(), nodes[1].get_center(), color=PURPLE, stroke_width=5),
            Line(nodes[1].get_center(), nodes[2].get_center(), color=PURPLE, stroke_width=5),
            Line(nodes[2].get_center(), nodes[3].get_center(), color=TEAL, stroke_width=5),
            Line(nodes[3].get_center(), nodes[4].get_center(), color=TEAL, stroke_width=5),
            Line(nodes[2].get_center(), nodes[5].get_center(), color=PURPLE, stroke_width=5),
            Line(nodes[5].get_center(), nodes[6].get_center(), color=TEAL, stroke_width=5),
        )
        legend = VGroup(
            Text("red ERROR", font=FONT, font_size=20, color=RED),
            Text("amber WARNING", font=FONT, font_size=20, color=AMBER),
            Text("blue INFO", font=FONT, font_size=20, color=BLUE),
            Text("purple = same request_id", font=FONT, font_size=20, color=PURPLE),
            Text("teal = shared resource_id", font=FONT, font_size=20, color=TEAL),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.18).to_edge(RIGHT, buff=0.55).shift(DOWN * 1.35)
        rule1 = Text("same request_id → strong correlation", font=FONT, font_size=27, color=PURPLE).move_to(LEFT * 3.25 + DOWN * 3.1)
        rule2 = Text("shared resource_id → resource correlation", font=FONT, font_size=27, color=TEAL).move_to(RIGHT * 2.45 + DOWN * 3.1)
    ...
    def graph_safety(self) -> None:
        ...
        circle_points = [np.array([2.0 * math.cos(i * TAU / 9), 2.0 * math.sin(i * TAU / 9), 0]) + LEFT * 3.7 for i in range(9)]
        messy_nodes = VGroup(*[make_graph_node("", AMBER, radius=0.18).move_to(p) for p in circle_points])
        messy_edges = VGroup(*[Line(circle_points[i], circle_points[j], color=SLATE, stroke_width=1.6, stroke_opacity=0.55) for i in range(9) for j in range(i + 1, 9)])
        messy_label = Text("naive: everything connects", font=FONT, font_size=24, color=AMBER).next_to(messy_nodes, DOWN, buff=0.4)
        clean_positions = [RIGHT * 1.2 + LEFT * 0.15 + RIGHT * i * 0.8 + UP * (0.5 * math.sin(i)) for i in range(7)]
        clean_nodes = VGroup(*[make_graph_node("", BLUE if i % 3 else RED, radius=0.2).move_to(clean_positions[i]) for i in range(7)])
        clean_edges = VGroup(*[Line(clean_positions[i], clean_positions[i + 1], color=CYAN, stroke_width=4) for i in range(6)])
        clean_label = Text("implemented: chronological and bounded", font=FONT, font_size=24, color=GREEN).next_to(clean_nodes, DOWN, buff=0.55)
        rules = VGroup(
            Text("consecutive edges only", font=FONT, font_size=24, color=WHITE),
            Text("periodic groups skipped", font=FONT, font_size=24, color=WHITE),
            Text("oversized groups bounded", font=FONT, font_size=24, color=WHITE),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.18).move_to(RIGHT * 4.2 + DOWN * 1.55)
    ...
    def incident_subgraph(self) -> None:
        ...
        edges = VGroup(*[Line(positions[a], positions[b], color=PURPLE if i % 2 else TEAL, stroke_width=4) for i, (a, b) in enumerate([(0, 1), (1, 3), (2, 3), (3, 4), (4, 5), (3, 6), (3, 7), (7, 8)])])
    ...
    def horizon_view(self) -> None:
        ...
        mini_edges = VGroup(Line(mini_nodes[0].get_center(), mini_nodes[1].get_center(), color=PURPLE, stroke_width=3), Line(mini_nodes[1].get_center(), mini_nodes[2].get_center(), color=TEAL, stroke_width=3), Line(mini_nodes[1].get_center(), mini_nodes[3].get_center(), color=PURPLE, stroke_width=3))

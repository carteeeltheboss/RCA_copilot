...
% Portable preamble: keep the deck compilable on a minimal TeX Live install.
% Required core packages only; optional packages get safe fallbacks.
...
% adjustbox is useful for scaling TikZ/table figures, but may be absent on
% minimal Debian/Ubuntu TeX installations. If missing, leave content unscaled
% instead of aborting compilation.
\makeatletter
\IfFileExists{adjustbox.sty}{%
  \usepackage{adjustbox}%
}{%
  \newenvironment{adjustbox}[1]{\par\begingroup\centering}{\par\endgroup}%
}
\makeatother
...
\titlegraphic{%
    \begin{tikzpicture}[remember picture,overlay]
        \node[anchor=north east, xshift=-0.55cm, yshift=-0.28cm]
            at (current page.north east) {\includegraphics[height=0.7cm]{logo}};
        \node[anchor=north west, xshift=0.55cm, yshift=-0.18cm]
            at (current page.north west) {\includegraphics[height=1.2cm]{huaweii}};
    \end{tikzpicture}%
}
\AtBeginSection[]{
  \begin{frame}
...
\slidepurpose{List all diagrams and figures presented in the report}{Navigation / global framing}
...
\begin{frame}{Segmentation of the Problem}
\slidepurpose{Break the central question into four distinct engineering sub-problems}{Scientific framing}
\centering
\begin{adjustbox}{max width=0.98\linewidth, max height=0.50\textheight}
\begin{tikzpicture}[>=Latex, font=\scriptsize, align=center,
    stage/.style={draw, thick, rounded corners=4pt, minimum width=2.4cm,
                  minimum height=1.6cm, inner sep=4pt, font=\scriptsize\bfseries},
    challenge/.style={font=\tiny, text width=2.6cm, align=center, gray!80!black},
    arr/.style={->, thick, line width=1.2pt, draw=gray!70}
]
    % Stage 1
    \node[stage, fill=blue!12] (s1) at (0,0)
        {1. Live\\Ingestion};
    \node[challenge, below=0.25cm of s1] (c1)
        {Stream from \texttt{journald}\\crash-safe cursor\\batch + ACK delivery};

    % Stage 2
    \node[stage, fill=violet!12] (s2) at (3.6,0)
        {2. Structuring\\\& Storage};
    \node[challenge, below=0.25cm of s2] (c2)
        {Regex parsing\\field extraction\\MongoDB persistence};

    % Stage 3
    \node[stage, fill=orange!12] (s3) at (7.2,0)
        {3. Graph\\Correlation};
    \node[challenge, below=0.25cm of s3] (c3)
        {Edge rules (req / resource)\\incident detection\\evidence package};

    % Stage 4
    \node[stage, fill=green!12] (s4) at (10.8,0)
        {4. AI\\Reasoning};
    \node[challenge, below=0.25cm of s4] (c4)
        {Embeddings + RAG\\local LLM (deepseek-r1)\\Chain-of-Thought};
    ...
    % Input / Output labels
    \node[font=\tiny\itshape, left=0.35cm of s1, text width=1.4cm, align=center] {raw live\\OpenStack logs};
    \node[font=\tiny\itshape, right=0.35cm of s4, text width=1.6cm, align=center] {explainable\\RCA answer};
...
{\footnotesize
\begin{itemize}
    \item \textbf{Stage 1:} logs never stop -- ingestion must be live, resumable, and crash-safe.
    \item \textbf{Stage 2:} raw text is unusable -- the parser extracts structured, queryable fields.
    \item \textbf{Stage 3:} ``what led to what'' is a graph problem, not a table filter.
    \item \textbf{Stage 4:} a local, open-weight model reasons over evidence -- never raw logs.
\end{itemize}}
\end{frame}
...
\slidepurpose{Show how the internship is split into work phases and iterations}{Organization / planning}
    ...
    \begin{tikzpicture}[node distance=2cm, auto]
        ...
        \node[draw, circle, fill=green!20, below of=sprint2, yshift=-1.5cm, align=center] (delivery) {Working\\Evidence\\Pipeline};
...
\begin{tikzpicture}[x=1.05cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
...
\centering
\begin{adjustbox}{max width=0.94\linewidth, max height=0.40\textheight}
\begin{tikzpicture}[x=1.15cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
...
\centering
\begin{adjustbox}{max width=0.94\linewidth, max height=0.40\textheight}
\begin{tikzpicture}[x=1.1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
...
\begin{tikzpicture}[>=Latex, font=\scriptsize, node distance=0.4cm and 0.5cm]
    \node[fill=gray!15, rounded corners, draw, font=\scriptsize\bfseries] (root) {RCA\_copilot/};
    \node[fill=blue!10, rounded corners, draw, below right=0.2cm and 0.5cm of root] (col) {collector/};
    \node[fill=blue!10, rounded corners, draw, below=0.1cm of col] (be) {backend/};
    \node[fill=violet!10, rounded corners, draw, below=0.1cm of be] (par) {parser\_worker/};
    \node[fill=violet!10, rounded corners, draw, below=0.1cm of par] (corr) {correlation\_worker/};
    \node[fill=orange!10, rounded corners, draw, below=0.1cm of corr] (inc) {incident\_worker/};
    \node[fill=orange!10, rounded corners, draw, below=0.1cm of inc] (enr) {enrichment\_worker/};
    \node[fill=green!10, rounded corners, draw, below=0.1cm of enr] (eng) {engine/};
    \node[fill=green!10, rounded corners, draw, below=0.1cm of eng] (scr) {scripts/};
    \node[fill=gray!8, rounded corners, draw, below=0.1cm of scr] (dc) {docker-compose.yml};
    \node[fill=gray!8, rounded corners, draw, below=0.1cm of dc] (req) {requirements.txt};
    \draw[->, thick] (root.south) |- (col.west);
    \draw[->, thick] (root.south) |- (be.west);
    \draw[->, thick] (root.south) |- (par.west);
    \draw[->, thick] (root.south) |- (corr.west);
    \draw[->, thick] (root.south) |- (inc.west);
    \draw[->, thick] (root.south) |- (enr.west);
    \draw[->, thick] (root.south) |- (eng.west);
    \draw[->, thick] (root.south) |- (scr.west);
    \draw[->, thick] (root.south) |- (dc.west);
    \draw[->, thick] (root.south) |- (req.west);
\end{tikzpicture}
\end{column}
\begin{column}{0.44\textwidth}
\vspace{0.3cm}
\centering
\includegraphics[width=5cm]{qrcode_ghub.png}\\[0.1cm]
{\captionsetup{type=figure,font=tiny,justification=centering}
\captionof{figure}{GitHub repository QR code}%
\recordfigureentry{GitHub repository QR code}}
{\tiny\textcolor{gray}{carteeeltheboss/RCA\_copilot -- MIT Licensed}}
\end{column}
\end{columns}
\end{frame}
...
\centering
\begin{adjustbox}{max width=0.94\linewidth, max height=0.42\textheight}
\begin{tikzpicture}[x=1.05cm,y=1cm, every node/.style={font=\tiny, align=center}]
...
\begin{frame}{Why DeepSeek-R1?}
\slidepurpose{Explain the rationale behind selecting DeepSeek-R1}{AI layer -- model choice}
\vspace{0.2cm}
\begin{columns}[c]
...
\begin{itemize}
    ...
    \item \textbf{Open Weights.}\\
    \item \textbf{Data Sovereignty.}\\
\end{itemize}
\end{column}
\begin{column}{0.55\textwidth}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{benchmark.jpg}\\[0.1cm]
{\captionsetup{type=figure,font=tiny,justification=centering}
\captionof{figure}{Official benchmark comparison (DeepSeek vs OpenAI o1)}%
\recordfigureentry{Official benchmark comparison}}
\end{column}
\end{columns}
\end{frame}
...
{\footnotesize
\begin{itemize}
    \item \textbf{Size calculation:} $8\text{B params} \times \frac{4\,\text{bits}}{8} = 4\,\text{GB (weights)}$ $+$ ${\sim}1\,\text{GB (KV cache + runtime + Context Window)}$ $\approx$ $\mathbf{6.6\,\textbf{GB}}$ $\subset$ $8\,\text{GB VRAM}$ \checkmark
    \item Quantized \texttt{deepseek-r1:8b} fits comfortably inside 8GB of VRAM with ${\sim}3$GB headroom.
...
\slidepurpose{Position what is delivered today against what remains ambitious}{Closure -- PoC status}
\centering
\begin{adjustbox}{max width=0.96\linewidth, max height=0.40\textheight}
\begin{tikzpicture}[x=1.0cm,y=1cm, every node/.style={font=\tiny, align=center}]
    \node[font=\bfseries\scriptsize, green!35!black] at (4.9,3.0) {Implemented (PoC)};
    \node[draw, rectangle, fill=green!12, minimum width=1.25cm] at (0,2.3) {ingestion};
    \node[draw, rectangle, fill=green!12, minimum width=1.25cm] at (1.45,2.3) {parsing};
    \node[draw, rectangle, fill=green!12, minimum width=1.35cm] at (2.95,2.3) {correlation};
    \node[draw, rectangle, fill=green!12, minimum width=1.25cm] at (4.5,2.3) {incidents};
    \node[draw, rectangle, fill=green!12, minimum width=1.55cm] at (6.1,2.3) {LLM RCA};
    \node[draw, rectangle, fill=green!12, minimum width=1.6cm] at (7.9,2.3) {Horizon UI};
    \node[draw, rectangle, fill=green!12, minimum width=1.8cm] at (9.85,2.3) {provider fwk};
    \node[font=\bfseries\scriptsize, gray] at (4.9,0.9) {Ambitious Improvement};
    \node[draw, rectangle, dashed, fill=gray!8, minimum width=2.4cm] at (3.4,0.2) {embeddings / RAG};
    \node[draw, rectangle, dashed, fill=gray!8, minimum width=2.4cm] at (6.4,0.2) {evaluation};
\end{tikzpicture}
\end{adjustbox}
\vspace{0.15cm}
{\footnotesize
\begin{itemize}
    \item This is a \textbf{PoC}: ingestion, parsing, correlation, incident construction, LLM generation and Horizon UI are implemented end to end.
    \item Embeddings / RAG and formal evaluation are not part of the current baseline; they remain an ambitious improvement track.
    \item The question now shifts from ``can the technology work?'' to ``how should it be deployed and operated inside OpenStack?''
\end{itemize}}
\end{frame}

\section{Context Window and Realistic Limits}

\begin{frame}{The Context Window Problem}
\slidepurpose{Explain why context size becomes a memory and deployment constraint}{AI reasoning -- limitations}
\centering
\begin{adjustbox}{max width=0.96\linewidth, max height=0.50\textheight}
\begin{tikzpicture}[>=Latex, x=1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
    % Same capacity view used for the model-fit slide, now applied to context growth.
    \draw[thick] (0,3.30) rectangle (8,4.10);
    \node[font=\tiny] at (4,4.33) {RTX 4070 Max-Q -- 8GB VRAM: fastest tier};
    \fill[violet!35] (0,3.30) rectangle (5.35,4.10);
    \fill[orange!35] (5.35,3.30) rectangle (6.85,4.10);
    \node[font=\tiny] at (2.65,3.70) {model weights + runtime $\approx$ 5--6GB};
    \node[font=\tiny] at (6.10,3.70) {KV cache\\context};
    \node[font=\tiny, gray] at (7.45,3.70) {headroom};

    \node[draw, rectangle, dashed, red!55!black, fill=red!6, minimum width=3.9cm] at (2.45,2.78)
        {longer context grows\\memory pressure};

    \draw[thick] (0,1.35) rectangle (8,2.12);
    \node[font=\tiny] at (4,2.34) {System memory -- 32GB RAM: offload tier};
    \fill[blue!25] (0,1.35) rectangle (8,2.12);
    \node[font=\tiny] at (4,1.74) {offloaded layers / cache fit here, but every token becomes slower than VRAM-only inference};

    \draw[thick] (0,0.12) rectangle (8,0.82);
    \node[font=\tiny] at (4,1.02) {SSD: durable evidence + checkpoints / swap fallback};
    \fill[gray!25] (0,0.12) rectangle (8,0.82);
    \node[font=\tiny] at (4,0.47) {keeps full logs, incidents and checkpoints; not active reasoning memory};

    \draw[->, thick, dashed] (6.25,3.30) -- node[right,font=\tiny]{overflow / offload} (6.25,2.12);
    \draw[->, thick, dashed, gray] (6.25,1.35) -- node[right,font=\tiny]{last resort} (6.25,0.82);
\end{tikzpicture}
\end{adjustbox}
\vspace{0.05cm}
{\footnotesize
\begin{itemize}
    \item Planning metric: one structured OpenStack log line is roughly \textbf{20--40 tokens}; \textbf{1,000 lines} can already be \textbf{20k--40k tokens} before instructions and answer space.
    \item A realistic RCA window is not ``all logs'': e.g. \textbf{6 services $\times$ 200 lines} is already \textbf{1,200 lines} ($\sim$\textbf{24k--48k tokens}).
    \item With \textbf{8GB VRAM}, the 8B model fits; \textbf{32GB RAM} can offload overflow; the \textbf{SSD} stores evidence, not live context.
\end{itemize}}
\end{frame}

\begin{frame}{Handling Context Realistically}
\slidepurpose{Define how the system bounds evidence without pretending context is unlimited}{AI reasoning -- limitations}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Bounded incident package}
{\footnotesize
\begin{itemize}
    \item Keep raw evidence in MongoDB / Horizon.
    \item Correlate first, then select evidence.
    \item Target: tens to hundreds of relevant lines, not thousands.
    \item Reserve output space for RCA generation.
\end{itemize}}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Realistic limitations}
{\footnotesize
\begin{itemize}
    \item No unlimited memory of every past log.
    \item Large prompts increase KV cache and latency.
    \item RAM offload helps capacity, not speed.
    \item Embeddings / RAG and evaluation stay future work.
\end{itemize}}
\end{block}
\end{column}
\end{columns}
\vspace{0.2cm}
\begin{alertblock}{Deployment Principle}
\centering\small Treat the LLM as a bounded reasoning worker. Keep durable evidence, checkpoints and investigation depth outside the context window; pass only the selected incident evidence into generation.
\end{alertblock}
\end{frame}

\section{After the PoC: Native OpenStack Component}



\begin{frame}{OpenStack Contribution Requirements}
\slidepurpose{Define the contribution bar before presenting the deployment architecture}{Integration branch -- upstream readiness}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Policy and packaging}
{\footnotesize
\begin{itemize}
    \item Service source should be \textbf{Apache License 2.0}, not MIT.
    \item No dependency should restrict redistribution or deployment.
    \item Dependencies should align with OpenStack global requirements.
\end{itemize}}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Operational conventions}
{\footnotesize
\begin{itemize}
    \item \texttt{pbr}/\texttt{setup.cfg}, console scripts and build artifacts.
    \item \texttt{oslo.config}, \texttt{oslo.log} and service config files.
    \item Keystone discovery/auth, \texttt{tox}, \texttt{pep8}, release notes and contributor docs.
\end{itemize}}
\end{block}
\end{column}
\end{columns}
\vspace{0.1cm}
{\tiny Source basis: OpenStack TC licensing, new-project requirements, Project Team Guide and Project Testing Interface.}
\end{frame}


\begin{frame}{From PoC to Deployable OpenStack Service}
\slidepurpose{Introduce the final deployment direction as a native OpenStack component}{Integration branch -- componentization}
\centering
\begin{adjustbox}{max width=0.96\linewidth, max height=0.40\textheight}
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
    \node[draw, rounded corners, fill=gray!10, minimum width=3.2cm, minimum height=1.25cm] (before) at (1.8,1.7)
        {docker-compose-only\\standalone app};
    \node[font=\Large, gray!70] at (4.35,1.7) {$\Rightarrow$};
    \node[draw, rounded corners, fill=blue!12, minimum width=3.9cm, minimum height=1.25cm] (after) at (7.25,1.7)
        {native OpenStack\\service component};
    \node[draw, rectangle, fill=violet!10, minimum width=2.8cm] at (1.8,0.25) {\texttt{pbr} packaging};
    \node[draw, rectangle, fill=teal!10, minimum width=3.2cm] at (4.95,0.25) {six console scripts};
    \node[draw, rectangle, fill=orange!10, minimum width=3.9cm] at (8.45,0.25) {\texttt{oslo.config} + \texttt{oslo.log}};
\end{tikzpicture}
\end{adjustbox}
\vspace{0.12cm}
{\footnotesize
\begin{itemize}
    \item After the PoC, the \texttt{openstack-integration} branch explores a different architecture: RCA Copilot becomes a deployable OpenStack component.
    \item \texttt{pbr} packaging means OpenStack-style Python packaging on top of \texttt{setuptools}: static metadata in \texttt{setup.cfg}, generated package metadata, wheel/sdist builds and installed entry points.
    \item Six console-script entry points define real service commands instead of ad-hoc container commands.
    \item Configuration and logging now match OpenStack service conventions: \texttt{/etc/rca-copilot/rca-copilot.conf}, \texttt{oslo.config} and \texttt{oslo.log}.
\end{itemize}}
{\tiny Source: \href{https://docs.openstack.org/pbr/latest/user/features.html}{OpenStack pbr documentation}.}
\end{frame}

\begin{frame}{Installed and Managed with OpenStack}
\slidepurpose{Show the enterprise deployment path instead of a separate application path}{Integration branch -- deployment model}
\centering
\begin{adjustbox}{max width=0.96\linewidth, max height=0.42\textheight}
\begin{tikzpicture}[>=Latex, x=1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
    \node[draw, rectangle, fill=gray!10, minimum width=2.7cm, minimum height=0.8cm] (local) at (0.8,1.8) {local.conf\\\texttt{enable\_plugin}};
    \node[draw, rectangle, fill=blue!12, minimum width=2.5cm, minimum height=0.8cm] (plugin) at (3.5,1.8) {DevStack\\plugin};
    \node[draw, rectangle, fill=green!12, minimum width=2.7cm, minimum height=0.8cm] (svc) at (6.4,2.7) {six systemd\\units};
    \node[draw, rectangle, fill=violet!12, minimum width=2.7cm, minimum height=0.8cm] (ks) at (6.4,1.5) {Keystone\\service + endpoints};
    \node[draw, rectangle, fill=orange!12, minimum width=2.7cm, minimum height=0.8cm] (hz) at (6.4,0.3) {Horizon\\five panels};
    \draw[->, thick] (local) -- (plugin);
    \draw[->, thick] (plugin) -- (svc);
    \draw[->, thick] (plugin) -- (ks);
    \draw[->, thick] (plugin) -- (hz);
    \node[font=\tiny, gray] at (9.35,2.7) {start / stop\\with stack lifecycle};
    \node[font=\tiny, gray] at (9.35,1.5) {public / internal / admin};
    \node[font=\tiny, gray] at (9.35,0.3) {verified live};
\end{tikzpicture}
\end{adjustbox}
\vspace{0.15cm}
{\footnotesize
\begin{itemize}
    \item A single \texttt{enable\_plugin} line in \texttt{local.conf} brings up the full service alongside core OpenStack services.
    \item The component is launched and stopped with the rest of the stack, rather than operated as a separate external application.
    \item Six systemd units use \texttt{Restart=on-failure} with backoff.
    \item Keystone service registration includes public, internal and admin endpoints; Horizon integration exposes five live panels.
\end{itemize}}
\end{frame}

\begin{frame}{Provider API in the Component Architecture}
\slidepurpose{Explain how the API changes when the project becomes an OpenStack-managed service}{Integration branch -- API architecture}
\centering
\begin{adjustbox}{max width=0.96\linewidth, max height=0.42\textheight}
\begin{tikzpicture}[>=Latex, x=1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
    \node[draw, rectangle, fill=orange!12, minimum width=2.2cm] (horizon) at (0.8,2.1) {Horizon\\panels};
    \node[draw, rectangle, fill=violet!12, minimum width=2.2cm] (ks) at (3.2,2.1) {Keystone\\token + roles};
    \node[draw, rectangle, fill=blue!12, minimum width=2.6cm] (api) at (5.9,2.1) {RCA Copilot\\provider API};
    \node[draw, rectangle, fill=green!12, minimum width=2.6cm] (workers) at (8.8,2.1) {workers +\\LLM generation};
    \node[draw, cylinder, shape border rotate=90, aspect=0.3, fill=teal!18, minimum width=2.3cm, minimum height=1.0cm] (mongo) at (5.9,0.65) {MongoDB\\evidence store};
    \draw[->, thick] (horizon) -- (ks);
    \draw[->, thick] (ks) -- (api);
    \draw[->, thick] (api) -- (workers);
    \draw[<->, thick] (api) -- (mongo);
    \draw[<->, thick] (workers) -- (mongo);
    \node[font=\tiny, gray] at (3.2,2.75) {RBAC gate};
    \node[font=\tiny, gray] at (5.9,2.75) {Keystone-registered endpoint};
\end{tikzpicture}
\end{adjustbox}
\vspace{0.12cm}
{\footnotesize
\begin{itemize}
    \item The API is no longer only a standalone app endpoint; it is treated as an OpenStack service endpoint discoverable through Keystone.
    \item Horizon calls the provider API through Keystone-role RBAC; ingestion is protected separately with rate limiting.
    \item The API coordinates incident retrieval, evidence storage and LLM generation while workers report heartbeats for operability.
\end{itemize}}
\end{frame}

\begin{frame}{Zero-Touch Startup}
\slidepurpose{Explain how the plugin removes manual database bootstrapping after stack.sh}{Integration branch -- startup automation}
\centering
\begin{adjustbox}{max width=0.95\linewidth, max height=0.42\textheight}
\begin{tikzpicture}[>=Latex, x=1cm,y=1cm, every node/.style={font=\scriptsize, align=center}]
    \node[draw, rectangle, fill=gray!10, minimum width=2.2cm] (stack) at (0.9,1.7) {\texttt{stack.sh}};
    \node[draw, rectangle, fill=blue!12, minimum width=2.8cm] (plugin) at (3.7,1.7) {RCA Copilot\\DevStack plugin};
    \node[draw, cylinder, shape border rotate=90, aspect=0.3, fill=teal!18, minimum width=2.4cm, minimum height=1.15cm] (mongo) at (6.8,1.7) {MongoDB};
    \node[draw, rectangle, fill=green!12, minimum width=2.5cm] (services) at (9.8,1.7) {services\\start cleanly};
    \draw[->, thick] (stack) -- (plugin);
    \draw[->, thick] (plugin) -- node[above,font=\tiny]{provision} (mongo);
    \draw[->, thick] (mongo) -- (services);
    \node[draw, rectangle, fill=orange!10, font=\tiny, align=center, minimum width=6.4cm] at (6.8,0.25)
        {auth + users + indexes + TTL are created by the plugin itself};
\end{tikzpicture}
\end{adjustbox}
\vspace{0.15cm}
{\footnotesize
\begin{itemize}
    \item MongoDB is auto-provisioned by the plugin itself: auth, users, indexes and TTL.
    \item No manual database steps are required after \texttt{stack.sh}.
    \item Startup becomes part of the OpenStack deployment path, not a separate operator checklist.
\end{itemize}}
\end{frame}

\begin{frame}{Production Hardening \& Quality Gates}
\slidepurpose{Summarize the controls needed to operate the service as more than a lab demo}{Integration branch -- hardening}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Runtime hardening}
{\footnotesize
\begin{itemize}
    \item MongoDB auth enforced.
    \item Keystone-role RBAC on the provider API.
    \item Rate limiting on ingestion.
    \item Per-worker heartbeats.
    \item TTL data retention.
\end{itemize}}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
{\footnotesize
\begin{itemize}
    \item \texttt{tox -e py311}: 81 tests passing.
    \item \texttt{pep8} passing.
    \item \texttt{reno} release notes added.
    \item \texttt{CONTRIBUTING.rst} added.
    \item wheel / sdist build verified.
\end{itemize}}
\end{block}
\end{column}
\end{columns}
...
{\footnotesize
\begin{itemize}
    \item This integration path remains isolated on \texttt{openstack-integration}; \texttt{main} is preserved as the original baseline.
\end{itemize}}
\end{frame}
...
\begin{alertblock}{Key Takeaway}
    The OpenStack RCA Copilot turns scattered, cross-service logs into a bounded, explainable incident graph -- reasoned over by a model that never leaves the building. The PoC now includes ingestion, correlation, LLM generation and Horizon UI; the next question is enterprise-grade OpenStack deployment, with RAG and formal evaluation kept as future improvement tracks.
\end{alertblock}
\centering
\vspace{0.4cm}
...
\begin{frame}{Sources / Webography (1/2)}
\slidepurpose{Collect the OpenStack governance and contribution references used}{References}
{\tiny
\begin{thebibliography}{99}
\bibitem{tc-license} OpenStack TC. \href{https://governance.openstack.org/tc/reference/licensing.html}{Licensing requirements}. Apache License 2.0 and dependency compatibility.
\bibitem{new-projects} OpenStack TC. \href{https://governance.openstack.org/reference/new-projects-requirements.html}{Requirements for new OpenStack Project applications}. Mission alignment, Four Opens, Keystone and project policies.
\bibitem{pti-python} OpenStack TC. \href{https://governance.openstack.org/tc/reference/pti/python.html}{Project Testing Interface: Python}. tox, pep8, coverage, docs, sdist and wheel interfaces.
\bibitem{team-guide} OpenStack Project Team Guide. \href{https://docs.openstack.org/project-team-guide/open-source.html}{Open Source}. Apache 2.0 and dependency guidance for projects.
\bibitem{reno} reno. \href{https://docs.openstack.org/reno/2.2.0/}{Release notes manager}. Release notes stored with code and reviewed with changes.
\end{thebibliography}
}
\end{frame}

\begin{frame}{Sources / Webography (2/2)}
\slidepurpose{Collect the implementation references used for the integration section}{References}
{\tiny
\begin{thebibliography}{99}
\bibitem{pbr} OpenStack pbr. \href{https://docs.openstack.org/pbr/latest/}{Python Build Reasonableness}. setuptools packaging, metadata generation and release-note integration.
\bibitem{pbr-entrypoints} OpenStack pbr. \href{https://docs.openstack.org/pbr/latest/user/using.html}{Usage: entry points}. Console-script entry point configuration.
\bibitem{devstack-plugins} DevStack. \href{https://docs.openstack.org/devstack/2023.1/plugins.html}{Plugins}. \texttt{enable\_plugin}, execution phases and lifecycle integration.
\bibitem{oslo-config} oslo.config. \href{https://docs.openstack.org/oslo.config/ussuri/configuration/format.html}{Configuration file format}. INI-style OpenStack service configuration.
\bibitem{oslo-log} oslo.log. \href{https://docs.openstack.org/oslo.log/latest/configuration/index.html}{Configuration Options}. Logging controlled through oslo.config options.
\bibitem{keystone-catalog} Keystone. \href{https://docs.openstack.org/keystone/latest/contributor/service-catalog.html}{Service Catalog Overview}. Service discovery and public/internal/admin endpoints.
\bibitem{horizon-plugin} Horizon. \href{https://docs.openstack.org/horizon/latest/contributor/tutorials/plugin.html}{Tutorial: Creating a Horizon Plugin}. Dashboard panel/plugin architecture.
\end{thebibliography}
}
\end{frame}

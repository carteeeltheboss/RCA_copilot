...
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[ok,minimum width=2.0cm] at (1.7,3.35) {Implemented\\ingestion};
  \node[ok,minimum width=2.0cm] at (3.95,3.35) {Implemented\\parsing};
  \node[ok,minimum width=2.0cm] at (6.2,3.35) {Implemented\\correlation};
  \node[ok,minimum width=2.0cm] at (8.45,3.35) {Implemented\\incidents};
  \node[ok,minimum width=2.0cm] at (10.7,3.35) {Implemented\\enrichment};
  \node[future,minimum width=2.0cm] at (2.85,1.75) {Future\\Horizon UI};
  \node[future,minimum width=2.0cm] at (5.25,1.75) {Future\\ChromaDB};
  \node[future,minimum width=2.0cm] at (7.65,1.75) {Future\\RAG};
  \node[future,minimum width=2.0cm] at (10.05,1.75) {Future\\AI answer};
  \draw[flow] (.75,3.35)--(11.75,3.35);
  \draw[futureline] (1.85,1.75)--(11.05,1.75);
  \node[note,text width=10.6cm] at (6,.56) {Solid boxes are implemented evidence construction. Dashed boxes are future explanation and operator-experience layers.};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[label,text=Slate] at (6,4.82) {Future Horizon RCA Copilot workspace};
  \node[future,minimum width=2.35cm,minimum height=2.75cm] at (1.85,3.0) {incident list};
  \node[future,minimum width=3.4cm,minimum height=1.85cm] at (5.0,3.55) {graph view};
  \node[future,minimum width=3.4cm,minimum height=1.08cm] at (5.0,2.0) {timeline};
  \node[future,minimum width=3.15cm,minimum height=2.75cm] at (9.35,3.0) {event details\\evidence summary};
...
\begin{frame}{15. Two-Machine Architecture}
\progress{Green}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[box,draw=DeepNavy,fill=Canvas,minimum width=5.65cm,minimum height=4.3cm] (mac) at (3.3,2.8) {};
  \node[label] at (3.3,4.7) {MacBook Ubuntu: implemented evidence plane};
  \node[service] at (1.55,3.75) {DevStack};
  \node[service] at (3.45,3.75) {collector/API};
  \node[storage] at (5.2,3.0) {MongoDB};
  \node[graph] at (2.6,2.1) {workers};
  \node[ok] at (4.4,1.55) {enriched\\incidents};
  ...
  \node[note,fill=white,inner sep=1pt,text width=2.05cm] at (6.88,4.12) {private API\\boundary};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[label] at (6,5.08) {End-to-end architecture: deterministic evidence first, AI later};
  \node[service] (os) at (1.0,4.0) {OpenStack};
  \node[storage] (j) at (2.6,4.0) {journald};
  \node[service] (col) at (4.15,4.0) {collector};
  \node[service] (api) at (5.85,4.0) {FastAPI};
  \node[storage] (raw) at (7.65,4.0) {raw};
  \node[transform] (parse) at (2.15,2.7) {parser};
  \node[storage] (parsed) at (3.75,2.7) {events};
  \node[graph] (corr) at (5.55,2.7) {correlation};
  \node[storage] (edges) at (7.25,2.7) {edges};
  \node[err] (inc) at (8.95,2.7) {incident};
  \node[ok] (en) at (10.65,2.7) {enrichment};
  \node[future] (hor) at (4.25,1.25) {Horizon};
  \node[future] (rag) at (7.1,1.25) {RAG/provider};
  \node[ok] (op) at (10.15,1.25) {operator};
  \begin{scope}[on background layer]
    ...
    \draw[futureline] (en)--(hor);
    \draw[futureline] (en)--(rag);
    \draw[futureline] (rag)--(op);
    \draw[futureline] (hor)--(op);
  \end{scope}
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future,minimum width=2.15cm] at (2.45,1.45) {Horizon UI};
  \node[future,minimum width=2.15cm] at (4.85,1.45) {vectors/RAG};
  \node[future,minimum width=2.15cm] at (7.25,1.45) {provider tests};
  \node[future,minimum width=2.15cm] at (9.65,1.45) {live demo};

...
config.background_color = "#071321"
...
DEEP_NAVY = COLORS["navy"]
PANEL = COLORS["panel"]
PANEL_2 = COLORS["panel_2"]
BLUE = COLORS["blue"]
CYAN = COLORS["cyan"]
...
SMALL_SIZE = 22
CAPTION_SIZE = 28
STROKE_WIDTHS = {"thin": 2, "normal": 3, "strong": 5, "glow": 8}
SPACING = {"caption": 0.38, "title_top": 0.35, "panel_radius": 0.12}
STROKE = STROKE_WIDTHS["normal"]
...
def make_title(number: int, title: str, subtitle: str | None = None) -> VGroup:
    return make_chapter_title(number, title, subtitle)
...
def make_storage(label: str, width: float = 2.2, height: float = 1.55) -> VGroup:
    return make_storage_cylinder(label, width, height)
...
def edge_between_nodes(source: VGroup, target: VGroup, color: str, stroke_width: float = 5, buff: float = 0.02, dashed: bool = False) -> VMobject:
    ...
    start = source_circle.get_boundary_point(direction) + unit * buff
    end = target_circle.get_boundary_point(-direction) - unit * buff
...
def make_boundary_edge(source: VGroup, target: VGroup, color: str, stroke_width: float = 5, buff: float = 0.02, dashed: bool = False) -> VMobject:
    return edge_between_nodes(source, target, color, stroke_width=stroke_width, buff=buff, dashed=dashed)


def graph_edge_between(source: VGroup, target: VGroup, color: str, stroke_width: float = 5, dashed: bool = False) -> VMobject:
    return edge_between_nodes(source, target, color, stroke_width=stroke_width, dashed=dashed)
...
def make_pipeline_arrow(start, end, color: str = CYAN, dashed: bool = False) -> VMobject:
    return make_arrow(start, end, color, dashed)


def make_dashboard_panel(label: str, width: float, height: float, color: str, center) -> VGroup:
    panel = RoundedRectangle(width=width, height=height, corner_radius=0.12, stroke_color=color, fill_color=PANEL, fill_opacity=0.84, stroke_width=STROKE)
    text = Text(label, font=FONT, font_size=22, color=color)
    text.move_to(panel.get_top() + DOWN * 0.32)
    return VGroup(panel, text).move_to(center)


def progress_indicator(active: int) -> VGroup:
    labels = ["Problem", "Structure", "Graph", "Incident", "Dashboard", "AI"]
    colors = [RED, BLUE, PURPLE, AMBER, TEAL, GREEN]
    dots = VGroup()
    for i, (label, color) in enumerate(zip(labels, colors)):
        dot = Circle(radius=0.055, stroke_color=color, fill_color=color if i <= active else PANEL_2, fill_opacity=1, stroke_width=1)
        txt = Text(label, font=FONT, font_size=13, color=color if i == active else MUTED)
        item = VGroup(dot, txt).arrange(RIGHT, buff=0.08)
        dots.add(item)
    dots.arrange(RIGHT, buff=0.28).to_edge(UP, buff=0.12).to_edge(RIGHT, buff=0.35)
    return dots
...
class RCACopilotExplainer(MovingCameraScene):
    """A cinematic mathematical-explainer animation for OpenStack RCA Copilot."""
    ...
    def intro(self) -> None:
        ...
        chain = Text("Logs  ->  Events  ->  Graph  ->  Incident Evidence  ->  Horizon  ->  Grounded AI", font=FONT, font_size=30, color=CYAN)
        chain.scale_to_fit_width(13.2)
    ...
    def problem(self) -> None:
        ...
        caption = make_caption("The failure is there. The problem is finding the evidence around it.")
    ...
    def parsing(self) -> None:
        ...
        fields = [
            ...
            ("source_raw_log_id", "raw:65fb..."),
        ]
        card = make_event_card("parsed event", fields, RED, width=5.15).move_to(DOWN * 0.9)
        ...
        caption = make_caption("Parsing adds structure and lineage while the raw record remains preserved.")
    ...
    def graph_safety(self) -> None:
        ...
        caption = make_caption("Naive correlation explodes; implemented correlation is chronological, bounded, and reviewable.")
    ...
    def enrichment(self) -> None:
        ...
        source = make_event_card("bounded subgraph", [("events", "37"), ("edges", "52"), ("seed", "nova ERROR")], PURPLE, width=3.7).move_to(LEFT * 5.35 + UP * 0.55)
        enrich = make_service_box("enrichment-worker", TEAL, width=3.0, font_size=24).move_to(LEFT * 1.25 + UP * 0.55)
        cards = VGroup(
            ...
            make_event_card("request IDs", [("req", "req-7d31...")], PURPLE, width=2.55),
            make_event_card("counts", [("event/edge", "37 / 52")], AMBER, width=2.55),
        ).arrange_in_grid(rows=2, cols=3, buff=0.18).move_to(RIGHT * 4.45 + UP * 0.15).scale(0.72)
        arrow1 = make_arrow(source.get_right(), enrich.get_left(), TEAL)
        arrow2 = make_arrow(enrich.get_right(), cards.get_left(), TEAL)
        caption = make_caption("Before AI, enrichment builds the deterministic evidence package.")
    ...
    def horizon_view(self) -> None:
        ...
        caption = make_caption("Node and timeline clicks update details without hiding the evidence chain.")
    ...
    def future_ai(self) -> None:
        ...
        explanation = make_event_card("grounded RCA explanation", [("input", "structured evidence"), ("status", "future capability")], AMBER, width=3.45).move_to(RIGHT * 2.55 + DOWN * 2.05).scale(0.84)
        ...
        providers = VGroup(*[Text(p, font=FONT, font_size=17, color=MUTED) for p in ["Ollama", "OpenAI-compatible", "Gemini", "Claude", "Custom HTTP"]]).arrange(DOWN, aligned_edge=LEFT, buff=0.08)
        providers.move_to(RIGHT * 5.75 + DOWN * 1.0)

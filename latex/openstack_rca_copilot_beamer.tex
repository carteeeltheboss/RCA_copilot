...
\tikzset{
  >=Latex,
  zone/.style={draw=DeepNavy,rounded corners=7pt,thick,fill=LightBG,inner sep=8pt},
  machine/.style={draw=DeepNavy,rounded corners=8pt,very thick,dashed,fill=LightBG,inner sep=8pt},
  trust/.style={draw=DeepNavy,rounded corners=8pt,thick,dash pattern=on 4pt off 2pt,fill=white,inner sep=7pt},
  svc/.style={draw=MediumBlue,very thick,rounded corners=4pt,fill=SoftBlue,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  worker/.style={draw=MediumBlue,very thick,rounded corners=4pt,fill=SoftBlue,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  transform/.style={draw=Teal,very thick,rounded corners=4pt,fill=SoftTeal,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  ok/.style={draw=MutedGreen,very thick,rounded corners=4pt,fill=SoftGreen,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  warn/.style={diamond,aspect=1.75,draw=Amber,very thick,fill=SoftAmber,align=center,text=DeepNavy,font=\small,inner sep=2pt},
  err/.style={draw=MutedRed,very thick,rounded corners=4pt,fill=SoftRed,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  graphlogic/.style={draw=Purple,very thick,rounded corners=4pt,fill=SoftPurple,align=center,text=DeepNavy,font=\small,minimum height=.72cm,inner xsep=6pt},
  ...
  future/.style={draw=Slate,thick,dotted,rounded corners=4pt,fill=white,align=center,text=Slate,font=\small,minimum height=.72cm,inner xsep=6pt},
  event/.style={circle,draw=Purple,very thick,fill=white,text=DeepNavy,font=\small,align=center,minimum size=.72cm},
  ...
  logflow/.style={-{Latex[length=2.6mm]},very thick,draw=MediumBlue},
  dbread/.style={-{Latex[length=2.4mm]},thick,dashed,draw=Slate},
  dbwrite/.style={-{Latex[length=2.6mm]},very thick,draw=Slate},
  control/.style={-{Latex[length=2.4mm]},thick,dash pattern=on 2pt off 2pt,draw=Amber},
  edgeReq/.style={-{Latex[length=2.4mm]},very thick,draw=Purple},
  edgeRes/.style={-{Latex[length=2.4mm]},very thick,dashed,draw=Teal},
  futureline/.style={-{Latex[length=2.4mm]},thick,dotted,draw=Slate},
...
\section{Section 1: Context and Problem}
...
\section{Section 2: Project Vision}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future] (rag) at (4.2,1.45) {Chroma/RAG};
  \node[future] (llm) at (7.2,1.45) {Local LLM};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[note,text width=8cm] at (6,.55) {The browser never calls Ollama directly; backend mediates future inference.};
\end{tikzpicture}
\end{frame}

\section{Section 3: Physical Deployment Topology}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future] at (9.7,1.35) {Ollama + local LLM};
...
\section{Section 4: Runtime Service Architecture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future] (ollama) at (9.4,1.8) {Ollama};
...
\section{Section 5: End-to-End Data Lineage}
...
\section{Section 6: Journald Ingestion}
...
\section{Section 7: Raw Data Storage}
...
\section{Section 8: Parsing Architecture}
...
\section{Section 9: Correlation as a Graph Problem}
...
\section{Section 10: Graph Explosion Defect and Fix}

\begin{frame}{Before: Complete Pairwise Graph}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \foreach \i/\ang in {1/90,2/30,3/-30,4/-90,5/-150,6/150}{
    \node[event] (e\i) at ({6+2.0*cos(\ang)},{2.55+1.55*sin(\ang)}) {E\i};
  }
  \foreach \i in {1,...,5}{
    \pgfmathtruncatemacro{\s}{\i+1}
    \foreach \j in {\s,...,6}{\draw[Purple!45,thin,-{Latex[length=1.4mm]}] (e\i)--(e\j);}
  }
...
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[graphlogic] (group) at (1.5,2.8) {group events\\by ID};
  \node[warn] (size) at (4,3.35) {size <= 100?};
  \node[warn] (periodic) at (6.7,3.35) {periodic?};
  \node[transform] (sort) at (9.2,3.35) {sort by time};
  \node[graphlogic] (edge) at (9.2,1.7) {emit consecutive\\edges};
  \node[err] (skip1) at (4,1.7) {skip oversized};
  \node[err] (skip2) at (6.7,1.7) {skip periodic};
  \draw[edgeReq] (group)--(size);
  \draw[control] (size)--node[above,note]{yes}(periodic);
  \draw[control] (size)--node[left,note]{no}(skip1);
  \draw[control] (periodic)--node[above,note]{no}(sort);
  \draw[control] (periodic)--node[right,note]{yes}(skip2);
...
\section{Section 11: Incident Seed Detection}
...
\section{Section 12: Incident Subgraph Construction}
...
\section{Section 13: Worker Checkpoint Defect and Fix}
...
\section{Section 14: Incident Enrichment}
...
\section{Section 15: MongoDB Collection Architecture}
...
\section{Section 16: Reliability Model}
...
\section{Section 17: Current State versus Future State}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \foreach \txt/\x/\y in {Chroma/7.5/3.2,embeddings/9.1/3.2,reranker/10.8/3.2,Ollama/8.2/2.1,Horizon/10.0/2.1}{\node[future,minimum width=1.4cm] at (\x,\y) {\txt};}
\end{tikzpicture}
\end{frame}

\section{Section 18: Selective Vectorization}

\begin{frame}{Selective Vectorization Funnel}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[db] at (1.2,3.1) {Mongo};
  \node[artifact,minimum width=2.3cm] at (3.5,3.6) {all raw logs};
  \node[warn,minimum width=2.5cm] at (5.8,3.0) {filter high\\signal};
  \node[future,minimum width=2.5cm] at (8.2,3.5) {embedding\\request};
  \node[future,minimum width=2.5cm] at (10.6,3.0) {Chroma\\vectors};
  \node[artifact] at (5.8,1.4) {ERROR\\WARNING\\traceback\\enriched incident\\summary\\correlated chunk};
  \draw[dbread] (1.8,3.1)--(3.0,3.6);
  \draw[control] (4.2,3.55)--(5.2,3.1);
  \draw[futureline] (6.5,3.0)--(7.5,3.5);
  \draw[futureline] (8.9,3.5)--(10.0,3.0);
\end{tikzpicture}
\end{frame}
...
\section{Section 19: MSI GPU Architecture}

\begin{frame}{Future MSI GPU Node}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[machine,minimum width=8.6cm,minimum height=4.4cm] at (6,2.45) {};
  \node[smalltitle,text=Slate] at (6,4.25) {MSI Ubuntu GPU node (future)};
  ...
  \node[future] (ollama) at (6.6,1.75) {Ollama\\local LLM};
  ...
  \node[note] at (6,.55) {MongoDB and Chroma stay on MacBook; MSI only computes embeddings/ranking/generation.};
\end{tikzpicture}
\end{frame}

\section{Section 20: RAG Investigation Flow}
...
\section{Section 21: Horizon Integration}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future] (msi) at (7.1,2.0) {MSI inference\\API};
  \node[future] (ollama) at (10,2.0) {Ollama};
...
\section{Section 22: Operator Investigation Journey}
...
\section{Section 23: Full Final Architecture}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[future,minimum width=1.75cm] at (1.8,1.45) {MSI embeds};
  \node[future,minimum width=1.75cm] at (3.8,1.45) {Chroma};
  \node[future,minimum width=1.75cm] at (5.8,1.45) {RAG};
  \node[future,minimum width=1.75cm] at (7.8,1.45) {Ollama};
